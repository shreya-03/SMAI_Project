...
def gini_of_attr(instances,target_index):
    val_freq = defaultdict()
    ...
    for record in instances:
        if (val_freq.has_key(record[target_index])):
            ...
            val_freq[record[target_index]]  = 1.0
    gini_val = 1.0 - pow((val_freq[1]/len(instances)),2)-pow((val_freq[0]/len(instances)),2)
    return gini_val

def gini_value(instances,attr_index,class_index=0):
    val_freq = {}
    data_subset=[]
    subset_gini_val = 0.0
    ...
    for val in val_freq.keys():
        ...
        subset_gini_val += val_prob * gini_of_attr(data_subset,0)
    return subset_gini_val
...
def choose_best_attr_gini_index(instances,candidate_attribute_indexes,class_index=0):
    gains_and_indexes = sorted([(gini_value(instances, i), i) for i in candidate_attribute_indexes], 
                               reverse=True)
    return gains_and_indexes[0][1]
